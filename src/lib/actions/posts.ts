...
export async function fetchPosts(cursor: string | null) {
  const fetchUrl = cursor
    ? `${process.env.BACKEND_URL}/posts?cursor=${cursor}`
    : `${process.env.BACKEND_URL}/posts`;

  const cookieStore = await cookies();
  const headersList = await headers();
  const token =
    headersList.get("x-access-token") || cookieStore.get("accessToken")?.value;
...
export async function fetchUserPosts(cursor: string | null, username: string) {
  const fetchUrl = cursor
    ? `${process.env.BACKEND_URL}/users/${username}/posts?cursor=${cursor}`
    : `${process.env.BACKEND_URL}/users/${username}/posts`;
...
export async function getPost(postId: string): Promise<ActionState | Post> {
  ...
  const fetchUrl = `${process.env.BACKEND_URL}/posts/${postId}`;
